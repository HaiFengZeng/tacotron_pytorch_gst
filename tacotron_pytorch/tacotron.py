...
class Decoder(nn.Module):
    def __init__(self, in_dim, r):
        ...
        self.attention_rnn = AttentionWrapper(nn.GRUCell(256 + 128 + 128, 256), BahdanauAttention(256))
        self.memory_layer = nn.Linear(256 + 128, 256, bias=False)
    ...
    def forward(self, encoder_outputs, inputs=None, memory_lengths=None):
        ...
        # go frames
        initial_input = Variable(encoder_outputs.data.new(B, self.in_dim * self.r).zero_())
        ...
        current_attention = Variable(encoder_outputs.data.new(B, 256 + 128).zero_())
        ...
        while True:
            ...
            attention_rnn_hidden, current_attention, alignment = self.attention_rnn(
                ...
                cell_state=attention_rnn_hidden,
...
class Tacotron(nn.Module):
    def __init__(self, n_vocab, embedding_dim=256, mel_dim=80, linear_dim=1025,
        ...
        self.gst_token = nn.Parameter(torch.randn(hparams.style_token, key_dim))  # required_grad=True
        # Trying smaller std
        self.embedding.weight.data.normal_(0, 0.3)
        self.encoder = Encoder(embedding_dim)
        self.decoder = Decoder(mel_dim, r)
        self.reference_encoder = ReferenceEncoder(input_dim=linear_dim, filters=[32, 32, 64, 64, 128, 128])
        self.style_attention = MultiHeadAttentionTest(query_dim=128, key_dim=key_dim, num_units=128)
    ...
    def forward(self, inputs,
        ...
        assert style_embedding.size() == (B, encoder_outputs.size(1), 128)
        # concat style embedding and encoder outputs
        _encoder_outputs = torch.cat((style_embedding, encoder_outputs), -1)
        # (B, T', mel_dim*r)
        mel_outputs, alignments = self.decoder(encoder_outputs=_encoder_outputs,
                                               inputs=targets,
